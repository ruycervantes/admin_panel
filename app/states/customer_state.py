...
class CustomerState(rx.State):
    ...
    def _processed_customers(self) -> list[CustomerData]:
        ...
        sorted_data = sorted(
            data_to_process, key=get_sort_value, reverse=self.sort_order == "desc"
        )
        return sorted_data
    ...
    async def download_csv(self):
        async with self:
            if not self._all_customers_data:
                ...
                yield rx.toast("No customer data to download.", duration=3000)
                return
            self.loading = True
            self.error_message = ""
        try:
            ...
            if not df_export_data:
                ...
                yield rx.toast("No customer data available to download.", duration=3000)
                return
            ...
            yield rx.toast(f"Error downloading CSV: {e!s}", duration=5000)
        finally:
            async with self:
                self.loading = False
