    ...
    return rx.radix.primitives.dialog.root(
        rx.radix.primitives.dialog.portal(
            rx.radix.primitives.dialog.overlay(
                class_name="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm"
            ),
            rx.radix.primitives.dialog.content(
                ...
                class_name="fixed top-1/2 left-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 p-4 z-50",
            ),
        ),
        open=CustomerState.show_edit_dialog,
        on_open_change=CustomerState.toggle_edit_dialog,
    )