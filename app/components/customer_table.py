import reflex as rx
from app.states.customer_state import CustomerData, CustomerState


def sortable_table_header_cell(text: str, column_key: str, icon: str) -> rx.Component:
    return rx.el.th(
        rx.el.div(
            rx.icon(tag=icon, size=16, class_name="text-gray-500"),
            text,
            rx.cond(
                CustomerState.sort_column == column_key,
                rx.icon(
                    tag=rx.cond(
                        CustomerState.sort_order == "asc", "chevron_up", "chevron_down"
                    ),
                    size=16,
...
def status_badge(status: str) -> rx.Component:
    return rx.el.div(
        rx.el.div(
            class_name=rx.match(
                status,
                ("Active", "h-1.5 w-1.5 rounded-full bg-teal-500"),
                ("Onboarding", "h-1.5 w-1.5 rounded-full bg-yellow-500"),
                ("Pending", "h-1.5 w-1.5 rounded-full bg-orange-500"),
                ("Suspended", "h-1.5 w-1.5 rounded-full bg-red-500"),
                ("Inactive", "h-1.5 w-1.5 rounded-full bg-gray-500"),
...
def customer_table_row(customer: CustomerData) -> rx.Component:
    return rx.el.tr(
        rx.el.td(
            rx.el.p(
...
def customer_table() -> rx.Component:
    return rx.el.div(
        rx.el.div(
            ...
            rx.el.div(
                rx.el.button(
                    rx.icon(tag="download", size=16, class_name="mr-1 sm:mr-2"),
                    rx.el.span("Download CSV", class_name="hidden sm:inline"),
                ...
                rx.el.button(
                    rx.icon(tag="plus", size=16, class_name="mr-1 sm:mr-2"),
                    rx.el.span("Add user", class_name="hidden sm:inline"),
        ...
        rx.cond(
            ...
            rx.el.div(
                    ...
                    rx.el.div(
                        rx.el.div(
                            rx.el.table(
                                ...
                                rx.el.tbody(
                                    rx.foreach(
                                        CustomerState.customers, customer_table_row
                                    ),
                ...
                rx.cond(
                    CustomerState.total_pages > 0, pagination_controls(), rx.fragment()
                ),
            ),
        ),
